...
struct SDParameters
{
    int p; // number of distinct eigenvalues
    std::vector<int> permutation; // permutation of eigenvalues in 3D case
    int flag;
};
...
public:
    ...
    static SDParameters SpectralDecomposition(const Matrix& X, Vector& e, std::vector<Matrix>& E)
    {
        ...
        if(TDimension == 2)
        {
            ...
            // handle the special case if X is null
            if(norm_frobenius(X) < zero_tol)
            {
                e.resize(1, false);
                e[0] = 0.0;

                E.resize(1);
                E[0] = IdentityMatrix(2, 2);
